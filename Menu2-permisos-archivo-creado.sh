...
Opcion7="Escribe el nombre de tu fichero sin extension txt $fichero.txt"

Opcion6="Salir"

# Creo el cuerpo del menú en cuestión , empezando de arriba a abajo ya que primero confeccioné la opción "Salir"

select opcion in "$Opcion1" "$Opcion2" "$Opcion3" "$Opcion4" "$Opcion5" "$Opcion7" "$Opcion6"; do
	if [ "$opcion" = "$Opcion6" ]; then
		...
		echo "Escribe el nombre de tu fichero  $fichero.txt"
		read fichero
		touch $fichero.txt
		chmod -xrw $fichero.txt
